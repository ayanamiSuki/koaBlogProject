<template>
  <div class="list">
    ...
    <el-card class="article-recommend">
      ...
      <el-row class="recommend-content" v-for="(item, index) in asideList" :key="'article' + index" @click.native="toDetail(item._id)">
        <em>{{ index + 1 }}</em>
    ...
    <el-dialog title="关于" :visible.sync="dialogVisible" width="30%">
      <div class="dialog-content">
        <div>
          <span class="main-color">ayanamiSuki</span>
          ，本质上是对二次元游戏[碧蓝航线[凌波]]中人物角色喜爱所以注册的网站
        </div>
        <div>
          <span class="bold">糖人</span>
          ，网站唯一神，服务器掌控者XD
        </div>
        <div>
          本人只是个卑微的有着ACG爱好的程序员，网站采用
          <span class="bold">vue +nuxt +koa +redis+mongodb</span>
          打造，单人打造正在磨合中
        </div>
        ...
        <div>
          唯一指定QQ
          <span class="delete_line">337845818</span>
          244802956
        </div>
      </div>
